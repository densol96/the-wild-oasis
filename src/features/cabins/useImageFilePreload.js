...
function useImageFilePreload(cabin, setValue, imageName) {
  const inputFieldUniqueId = "image" + (cabin ? cabin.id || "one-type" : "");
  useEffect(() => {
    if (cabin) {
      ...
      (async () => {
        ...
        if (imgUrl) {
          ...
          const file = new File([blob], imageName || imgUrl.split("/").at(-1), {
            type: blob.type,
          });
